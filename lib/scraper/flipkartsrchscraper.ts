...
const proxifly = new (require('proxifly'))({
    apiKey: 'FSTbMtKqnZJcDEdga56BDjVsNsUvyF8phmLFtctKxxbx'
  });

export default async function flipkartsrchscraper(url: string) {
    if (!url) return;

    let mainurl =encodeURIComponent(url)
    console.log(mainurl);
    ...
    try {
        ...
        console.log(url);
         ...
         const response = await axios.get('https://www.flipkart.com/search?q=tablet', options)
